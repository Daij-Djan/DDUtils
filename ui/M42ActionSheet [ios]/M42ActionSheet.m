...
- (void)animationDidStop:(NSString *)animationID finished:(NSNumber *)finished context:(void *)context {
    if ([animationID isEqualToString:@"removeFromSuperviewWithAnimation"]) {
		...
		NSUInteger buttonIndex = [(NSNumber*)context integerValue]; 
...
//	defaultButtonColorName = [[UIDevice currentDevice] isIPad] ? @"white" : @"lightGray";
	...
	UIImage *image = [M42CompatibleImage imageNamed:@"sheet_header.png"];
	...
	image = [M42CompatibleImage imageNamed:@"sheet_footer.png"];
	if(image) {
...
- (void)setButtonAtIndex:(NSUInteger)i colorName:(NSString*)colorName {
	...
	UIImage *buttonBackground = [M42CompatibleImage	imageNamed:[NSString stringWithFormat:@"button_%@.png", colorName]];
	UIImage *buttonBackgroundPressed = [M42CompatibleImage imageNamed:@"button_blue.png"];
...
- (void)setDestructiveButtonColorName:(NSString*)colorName {
	if(destructiveButtonColorName != colorName) {
		[destructiveButtonColorName release];
